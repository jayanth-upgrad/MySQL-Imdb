...
select count(id) as title_null
from movie
where title is null;

select count(id) as year_null
from movie
where year is null;

select count(id) as date_published_null
from movie
where date_published is null;

select count(id) as duration_null
from movie
where duration is null;

select count(id) as country_null
from movie
where country is null;

select count(id) as worlwide_gross_null
from movie
where worlwide_gross_income is null;

select count(id) as languages_null
from movie
where languages is null;

select count(id) as production_company_null
from movie
...
where year = 2019
group by country
having country like '%India%' or country like '%USA%'
order by number_of_movies desc
)
select sum(number_of_movies) as total_movies
from movie_2019;
...
-- Type your code below:
select genre, title, count(title) as generic_movies
from movie as m
inner join genre as g
on m.id = g.movie_id
group by title
having generic_movies = 1;
...
select round(min(avg_rating),0) as min_avg_rating, round(max(avg_rating),0) as max_avg_rating,
...
select title, median_rating, date_published
from movie as m
inner join ratings as r
on m.id = r.movie_id
...
on m.id = r.movie_id
group by country
having country = 'Italy' or country = 'Germany';
...
select count(id) as name_nulls
from names
where name is null;

select count(id) as height_nulls
from names
where height is null;

select count(id) as date_of_birth_nulls
from names
where date_of_birth is null;

select count(id) as known_for_movies_nulls
from names
...
with top3_genres as (
	select genre, count(r.movie_id) as movie_counts
	from genre as g
	inner join ratings as r
...
on r.movie_id = g.movie_id,
top3_genres
where g.genre in (top3_genres.genre) and avg_rating > 8
...
+---------------+-------------------+---------------------+----------------------+-----------------+*/
-- Type your code below:
select name as actor_name, sum(total_votes) as total_vote, count(r.movie_id) as movie_count,
avg(avg_rating) as actor_avg_rating, dense_rank() over(order by avg(avg_rating) desc) as actor_rank
from names as n
...
select genre, avg_duration, sum(avg_duration) over(order by genre) as running_total_duration, avg(avg_duration) over(order by genre) as moving_avg_duration
...
select year, title as movie_name, worlwide_gross_income,
...
select name as actress_name, sum(total_votes) as total_vote, count(title) as movie_count, avg(avg_rating),
